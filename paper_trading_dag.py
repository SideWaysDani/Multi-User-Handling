...
# Paper 2
# Key :PKFW6JQZ201FHYBWFM5C
# Secret: vIOXD497aceQtIQAl41cpoyCDUMLUv8xXfRpJbNE

# Paper 3
# Key: PKEXRCBONN22KAM81303
# Secret: 0wpcbFv9fXQdVTxe8PI5Gop561TU4zvVilIHl14c



# account name = Paper 3
api_key="PKEXRCBONN22KAM81303"
secret_key="0wpcbFv9fXQdVTxe8PI5Gop561TU4zvVilIHl14c"
paper =  True
...
class StockAnalyzerUsingAzureAPI:
    ...
    @staticmethod
    def load_data_from_api(st_name, start_date="2023-01-01", end_date="2025-12-31"):
...
@dag(dag_id='paper_trading_dag', schedule_interval='30 14 * * *', tags=['paper_trading_dag'], render_template_as_native_obj=True, catchup=False, is_paused_upon_creation=False, max_active_tasks=1,  default_args=default_args)
def paper_trading_dag():
    ...
    def insert_into_allocation_history(conn, values_list: list[list]):
        ...
        db_helper.insert(
            ...
            columns=['allocated_strength',	'opening_price',
                     ...
                     'valid_from_start_date',	'valid_to_end_date',	'unit_assignment_id', 'allocation_id', 'status', 'battle_date','deployment_id', 'utilized_strength'
                     ],
    ...
    def get_lead_name_from_deployment(db_helper: GenericDBHelper, depl_id):
            result = db_helper.select_all(
                columns='leads_gold_ml.id, leads_gold_ml.stock_name',
                table_name=f'deployment JOIN stocktrader.leads_gold_ml ON {schema_name_global}.deployment.lead_id = stocktrader.leads_gold_ml.id',
                where_clause=f'deployment.deployment_id = %s',
                where_values=(depl_id,)
            )

            leads_id = result[0][0]
            leads_name = result[0][1]
            return leads_id, leads_name
    ...
    def insert_into_allocation(conn, values_list, battle_date_for_allocation_history):  # working
        ...
        print("Inserting into allocation table Deployemnt ID", deployment_id)

        lead_id, lead_name = get_lead_name_from_deployment(
        ...
        try:
            ...
            prev_qty = None
            #getting the current postion of the lead if any , before creating a buy order
            positions = trade_client.get_all_positions()  
            print("Position of ", lead_name, " before creating buy order")
            if not positions:
                ...
                for pos in positions:
                    if pos.symbol== lead_name:
                        prev_qty = pos.qty
                        print('Position Fetched: ',pos.symbol,' : Qty: ', prev_qty)
                        avg_entry_price = pos.avg_entry_price
                        print('Position Fetched: ',pos.symbol,' : Avg Entry Price: ', avg_entry_price)
            ...
            print("Position of ", lead_name, " after filling buy order")
            ...
            if not positions:
                ...
                for pos in positions:
                    if pos.symbol== lead_name:
                        qty = pos.qty
                        print('Position Fetched: ',pos.symbol,' : Qty: ', qty)
                        avg_entry_price = pos.avg_entry_price
                        print('Position Fetched: ',pos.symbol,' : Avg Entry Price: ', avg_entry_price)

            if prev_qty:
                stock_quantity = Decimal(qty) - Decimal(prev_qty)
                print("stock quantity of current allocation: ",qty," - ",prev_qty," = ", stock_quantity)
            else:
                stock_quantity = Decimal(qty) 
                print("stock quantity of current allocation: ",qty)    
            opening_price = order.filled_avg_price
            utilized_strength = Decimal(stock_quantity) * Decimal(opening_price)
            ...
            values_list[0][2] = opening_price
            values_list[0][3] = closing_price
            values_list[0][5] = stock_quantity 
            values_list[0][7] = utilized_strength
            ...
            value_list = [[allocated_strength, opening_price, lead_id, closing_price,
                           stock_quantity, p_l, valid_from_start_date, valid_to_end_date, unit_assignment_id, allocation_id, status, battle_date_for_allocation_history,deployment_id, utilized_strength]]
    ...
    def create_buy_working_order(symbol_to_buy, qty):
        

        print(f'Creating Buy Working Order for {symbol_to_buy} with quantity {qty}')
    ...
    def is_order_filled(order_id):
        ...
        if (order.filled_avg_price is not None):
            print(f"Order filled {order.id} of {order.symbol} for {order.order_type} with filled_avg_price {order.filled_avg_price}")
            return True
    ...
    def update_allocation(conn, allocation_id, columns_to_be_updated: list, new_values: list, start_date_for_allocation_history: date, end_date_for_allocation_history: date, current_battle_date):  # working
        ...
        values_list = [[allocated_strength, opening_price, lead_id, closing_price,
                        stock_quantity, p_l, valid_from_start_date, valid_to_end_date, unit_assignment_id, allocation_id, status, current_battle_date,deployment_id,utilized_strength]]
        print(
    ...
    def process_allocations_for_removing_them(allocations_to_be_removed: list, allocation_history_status: str, battle_date):
        ...
        for poor_performer in allocations_to_be_removed:
            ...
            allocation_id, profit_and_loss, deployment_id, opening_price, closing_price, allocated_strength, stock_quantity, allocation_status ,utilized_strength= poor_performer
            print("hello 3")
            ...
            if deployment_result:
                ...
                closing_price = Decimal(order.filled_avg_price)
                sold_quantity = Decimal(order.filled_qty)

                print("Closing price after sell:",closing_price)
                profit_and_loss = (closing_price - opening_price)*sold_quantity
                print("profit and loss after sell:",profit_and_loss)


                utilized_strength = allocated_strength
                ...
                allocation_history_values_list = [[allocated_strength, opening_price, lead_id, closing_price,
                                                   stock_quantity, profit_and_loss, start_date, end_date, unit_assignment_id, allocation_id, allocation_history_status, battle_date,deployment_id, utilized_strength]]
                ...
                profit_and_loss_percent = calculate_profit_and_loss_percent(opening_price, closing_price, stock_quantity)

                performance_values_list = [
                        (unit_assignment_id, battle_date, profit_and_loss, lead_id, start_date, battle_date, allocation_id, profit_and_loss_percent)]
                ...
                print("Current Cash level that is going to become the new remaining strength: ",acct.cash)

                remaining_strength = Decimal(acct.cash)
                total_strength = Decimal(active_strength) + Decimal(remaining_strength)
    ...
    #     result = db_helper.select_all(columns='allocation.*,leads_gold_ml.stock_name,leads_gold_ml.leads_gold_ml_id', table_name=f'''allocation JOIN  {schema_name_global}.deployment ON allocation.deployment_id = deployment.deployment_id
    #                           JOIN  {schema_name_global}.leads_gold_ml ON deployment.lead_id = leads_gold_ml.leads_gold_ml_id''')
    #     if result:
    #         # print("result", result)
    ...
    def remove_allocation_to_sell_using_StockAnalyzerUsingAzureAPI(dbhelper: GenericDBHelper, conn, battle_date):
        ...
        if lead_and_allocations_data:
            ...
            print('Current Positions after Selling:\n')
            ...
            if not positions:
                ...
                for pos in positions:
                    total_allocated += Decimal(pos.qty) * Decimal(pos.avg_entry_price)
                    print(f"Position: {pos.symbol}, Qty: {pos.qty}"
                        f" Avg Entry Price: {pos.avg_entry_price} , Current Price: {pos.current_price} ,sotck quantity: {pos.qty_available} ,cost basis : {pos.cost_basis}")
            ...
            if not open_orders:
                ...
                for order in open_orders:
                    print(f"Order ID: {order.id}, Symbol: {order.symbol},  "
                        f"Qty: {order.qty}, Status: {order.status}, Type: {order.order_type}, filled at {order.filled_at}")
    ...
    def get_trending_leads(conn, current_date, start_date, end_date):
        ...
        query = """
                ...
                WHERE leads_gold_ml.lead_date BETWEEN %s AND %s 
                ORDER BY leads_gold_ml.lead_date DESC
                """
        ...
        if one_to_one_flag:
            print("One to one flag is ",one_to_one_flag)
            print("fitlering leads for one to one correspondance")
        ...
        if origin == 'trends':
            ...
            for stock_id, stock_name in sorted_data:
                ...
                values_list_leads.append(
                            (stock_id, stock_name, battle_date, 1, 1))
    ...
    #     table_name = f' {schema_name_global}.leads_gold_ml'
    ...
    def analysing_units_to_assign_leads(conn, unassigned_units, battle_date):
        ...
        strength_to_allocate_each_unit = (total_remaining_strength / num_unassigned_units) * Decimal("0.97")
        # ----------------------------------------------------------
    ...
    def get_sectors_from_trending_leads(conn, trending_leads):
        ...
        Args:
            ...
            trending_leads: A list of trending leads_gold_ml.
    ...
    def liquidate(conn, battle_date):
        """
        Fetches all allocations from paper_trading_test.allocation and processes them for removal.
        """
        query = "SELECT * FROM paper_trading_test.allocation"
        try:
            with conn.cursor() as cur:
                cur.execute(query)
    ...
    #Call the procedure to get control_flags
    ...
    def reset_positions():
        ...
        print("After closing all positions")
        # Fetch all open positions
        positions = trade_client.get_all_positions()
        total_allocated = 0  
        while positions:
            for pos in positions:
                total_allocated += Decimal(pos.qty) * Decimal(pos.avg_entry_price)
                print(f"Position: {pos.symbol}, Qty: {pos.qty}"
                    f" Avg Entry Price: {pos.avg_entry_price} , Current Price: {pos.current_price} ,total entry price: {pos.qty_available} ,cost basis : {pos.cost_basis}")
                if pos.symbol== 'ADI':
                    qty = pos.qty
    ...
    @task()
    def process_units( date_str: str):
        ...
        check_for_account_strength_update(
            conn=conn,
            db_helper=db_helper,
            account_id=hardcoded_account_id,
            current_battle_date=current_date
        )
        ...
        #     continue
            ...
            if len(trending_leads) > 0:
                ...
                print("Data received from polygon: ", polygon_data)
        ...
        if not unassigned_units:
            print("No unassigned units found. Skipping fetching leads_gold_ml.")
            #leads_data_from_table = []
        ...
        for unit in total_units:
            ...
            if unit_assignment_status == 'assigned':
                    ...
                    if not positions:
                        ...
                        for pos in positions:
                            ...
                            if pos.symbol == lead_name:
                                ...
                                opening_price_of_position = float(pos.avg_entry_price)
                                stock_quantity_of_position = float(pos.qty)
                                ...
                                opening_price_for_current_date = opening_price_of_position
                                print("in condition opening_price_for_current_date",
                                        opening_price_for_current_date)
                    
                    updated_utilized_strength = allocation_data[8]
                ...
                update_allocation(conn, allocation_id=allocation_id_for_update, columns_to_be_updated=['profit_and_loss', 'opening_price', 'closing_price','utilized_strength'],
                                    new_values=[profit_and_loss, opening_price_for_current_date, closing_price_for_current_date,updated_utilized_strength], start_date_for_allocation_history=start_date_depl, end_date_for_allocation_history=end_date_depl, current_battle_date=battle_date)
                ...
                #To math floor to 2 deciaml places
                strength = math.floor(strength_to_allocate_each_unit * 100) / 100
                current_date = battle_date
                account_id = hardcoded_account_id


                # Getting the current price of the lead
                ...
                if response.status_code == 200:
                    data = response.json()
                    current_price = data['trade']['p']  # 'p' is the price field in the response
                    print(f"Current price for {symbol}: {current_price}")
                else:
                    print(f"Failed to get latest trade for {symbol}. Status code: {response.status_code}")
                    print(response.json())
                ...
                try:
                    ...
                    # print("DEBUG - Alpaca SDK version:", alpaca.__version__)

                    # print("DEBUG - latest_trade_request =", latest_trade_request)
                    ...
                    print("strength used for caculating stock quantity",
                        strength)
                    ...
                    utilized_strength = stock_quantity * opening_price
                    

                    values_list = [[profit_and_loss, deployment_id, opening_price,
                                    closing_price, strength, stock_quantity, 'materlized',utilized_strength]]
                    ...
                    print(f"Error in allocating {unit_assignment_id}, lead {symbol}: {e}")
                    raise
    ...
    def check_market_status():
        ...
        if not clock.is_open:
            print("Market is closed. Skipping DAG.")
            raise AirflowSkipException("Market is closed.")
...
# Assuming you have a datetime object named 'current_date'
    ...
    #reset_positions = reset_positions.override(task_id=f"reset_positions")()

    market_check = check_market_status.override(task_id=f"check_market_status")()
    azure = remove_by_azure.override(task_id=f"process_sell_signals_sells")(d)
    battle = process_battleday_logic.override(task_id=f"process_battleday_logic")(d)
    ctrl = process_control_flag.override(task_id=f"process_control_flag")(d)
    units = process_units.override(task_id=f"process_units")(d)
    summary = fill_summary.override(task_id=f"process_summary_filling")(d)

    # Updated chaining
    market_check >> azure >> battle >> ctrl >> units >> summary
